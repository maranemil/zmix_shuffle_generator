...
ffmpeg -i load/in.wav -acodec pcm_s16le -ac 1 -ar 8000  output/out.wav
...
for i in {1..9}; do rubberband -c $(shuf -i0-3 -n1)  -t $(shuf -i0-2 -n1)  -T $(shuf -i0-2 -n1)   -p $(shuf -i0-8 -n1) output/out.wav output/mono_$i.wav ;done

# for i in mono_*.wav; do echo $i; done

for i in output/mono_*.wav; do ffmpeg -i "$i" -t 1 -y output/stereo_"$i"; done

rm list.txt && (for i in output/stereo*.wav; do echo file $i >> list.txt; done) && ffmpeg -safe 0 -f concat -i list.txt -c copy output/output_$(date +%s).wav